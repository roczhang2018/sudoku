...
input.addEventListener('focus',()=>highlightPeers(r,c,true));input.addEventListener('blur',()=>highlightPeers(r,c,false));cell.appendChild(input);boardEl.appendChild(cell);}}}
...
function renderCounts(){if(!countsEl) return;const counts=countDigits();let html='';for(let n=1;n<=9;n++){const remaining=9-counts[n];const zeroClass=remaining===0?' class="zero"':'';html+=`<li${zeroClass}><span>${n}</span><span>${remaining}</span></li>`;}countsEl.innerHTML=html;}
